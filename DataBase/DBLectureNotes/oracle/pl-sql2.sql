...
DECLARE --선언
  vno number(4); -- 헐 여기가 콤마가 아니라 세미콜론이네;;
  vname varchar2(20);
BEGIN
...
BEGIN
  select empno , job ,sal , deptno
    into v_empno, v_job , v_sal , v_deptno -- plsql은 기본적으로 데이터 한개만  담을 수 있음.(=질의는 하나의 행만 RETURN 해야 합니다.) (커서 라는 개념배우면 가능)
  from emp
  where ename = v_name;
...
/*
   DECLARE
          CURSOR 커서이름 IS 문자(커서가 실행할 쿼리) -- 커서도 변수임
   BEGIN
...
BEGIN
    ...
    LOOP  --데이터 row 건수 만큼 반복
      ...
      /*
        ...
        7900 JAMES 950
      */
      FETCH c1 INTO vempno , vename, vsal; -- 커서가 가리키는 로우를 변수에 담음
...
BEGIN
   -- FOR .. IN .. 으로 커서 OPEN, FETCH 생략. 
    FOR emp_record IN emp_curr  --row 단위로 emp_record변수 할당 (emp_record 마치 객체 처럼!)
    LOOP
...
-------------------------------------------------
DECLARE
  v_sal_total NUMBER(10,2) := 0; -- 초기값 0
  CURSOR emp_cursor             -- 커서이름 emp_커서
...
BEGIN
  ...
  FOR emp_record IN emp_cursor -- for int emp_record : emp_cursor 같은 for문임 
                               -- select한 데이터를 받는 변수 emp_record와 커서이름 emp_cursor
  LOOP
      v_sal_total := v_sal_total + emp_record.sal; -- row를 거듭할수록 월급 누적
  ...
  BEGIN
    ...
    LOOP
        ...
        IF(vemp.deptno = 20) THEN 
              ...
              values (vemp.empno,vemp.ename, vemp.sal,vemp.deptno,vemp.comm,result); -- cursor테이블의 totalsum컬럼에result(com이랑 sal계산한거) 넣음.
...
is
  BEGIN
    ...
    set sal = 0
    where empno = vempno;
    -- 파라미터로 받은 사번을 가진 사원의 급여를 0으로 만드는 프로시져
  END;
--실행방법
...
exec usp_getemplist(7902); -- 프로시저usp_getemplist에 파라미터로 7902를 넣어 실행
...
(
  vempno IN emp.empno%TYPE, --  vempno emp.empno%TYPE, -- IN : 생략 해서 잘 씀
...
BEGIN
   app_get_emplist(7902,out_ename,out_sal); -- IN은 param값 넣어주고, OUT써있는것은 리턴값 담을 변수 넣어줘야 한다.
...
프로시저에서 레퍼런스 변수를 사용하면 결과 값을 리턴할 수 있다.
*/
create or replace procedure usp_getEmpList
(
...
return number   -- 리턴타입-- public int f_max_sal(int deptno) {  return 10}
is
...
BEGIN
      ...
      return max_sal;
END; -- 사용자 정의 함수는 프로지서와 다르게 SELECT,FROM,WHERE와 함께 사용한다. (프로시저는 독자적인것 가능)
...
--AFTER :  테이블에서 DML 실행후에 트리거 동작 -- 이걸 많이 씀
...
delete from tri_emp where empno=100;
--------------------------------------- 여기 ----------------------------------------------------------------
...
select * from emp_audit; -- 1	KOSA	updating	2023/04/26 15:49:29 3개행을 수정해도 기록은 audit 테이블에 1개 남음.
                         -- for each row를 걸면 세 건반영시 기록도 3건 생김 
delete from emp2 where deptno = 20; -- 8개 행 삭제
...
--:OLD > 트리거가 처리한 레코드의 원래 값을 저장 (ms-sql (deleted)   -- 과거, 기존 데이터
--:NEW > 새값을 포함                             (ms-sql (inserted)  -- 현재 데이터

create or replace trigger tri_order2
before insert on tri_order
for each row
BEGIN
  IF(:NEW.o rd_code) not in('desktop') THEN
