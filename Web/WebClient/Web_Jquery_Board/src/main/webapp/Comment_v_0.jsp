		...
		$(function(){
			$('#addBtn').on("click",function(){
				if($.trim($('#comment').val()) == "")
				{
					...
					return false; 
					/*
					return false;는 이벤트 핸들러를 종료시키고, 이벤트의 기본 동작을 취소하는 역할을 합니다. 
					이 경우에는 클릭 이벤트를 처리하는 핸들러 함수가 종료되며, false를 반환하므로 
					이벤트의 기본 동작(일반적으로는 폼 전송 등)이 발생하지 않게 됩니다. */
				}
				...
				$.ajax(
						{
							...
							{
								$('#container').html(htmldata); // 성공응답에서 받아온 데이터를 container안에 뿌려줌
								$('#comment').val(""); // input 안 비워줌
							}
						}	
				      );
