...
<% // script 릿?
	...
	// jsp 장점 : 쉬워요 ... 모든 개발자 쉽게 웹을 ...
	//     단점 : 분업화 힘들어요 , 유지보수 (모든 코드가 한곳에) >> 관심사가 분리되어있지 않아요.
	
	Date date = new Date();
	
	/* 
		JSP 페이지 : UI(html+css+javascript) + JAVA코드(<% 스크립트 안에 구현)
		>> 디자인코드 + 서버의 로직코드 구현 : 문제점 > 협업, 유지보수 하기 어려워요
				
		servlet(형) > 웹 전용 java 파일 > jsp(동생)
		>> java 웹 기바 기술이 아니에요. > 계산기, 그림판, 윔도우 프로그램(awt, swing) > 웹 전향(객체:request, response)
		>> java 페이지 Web을 서비스 하자 > login.java > loginok.java(웹객체 상속 ...) > 웹에서 사용하는 자바(servlet)
		>> 단점 : 배우기 어렵다, 디자인 작업 (힘들어요), 협업(x)
		
			[ 서블릿에서 디자인 코드 ]... reponse.getWriter().print("<body>") ... 이런식...
		>> JSP 탄생 >> 장점 >> UI 작업 >> web page >> html base >> 디자인 good >> 단점 java 코드 <% .. dksdp
	    >> 혼재 (JAVA + UI) >> 힘들어 ....
			
		정리)
		서로의 장점을 살리자 (현대적인 프로그래밍은 잘하는 것을 키워주자, 업무의 효율 ) >> MVC  패턴 도입
